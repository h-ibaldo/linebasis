...
test.describe('Hover Border for Grandchildren of Rotated Auto-Layout', () => {
	test('hover border should match element position for grandchildren', async ({ page }) => {
		...
		if (positions && positions.hoverBorder) {
			const mismatchX = Math.abs(positions.hoverBorder.canvas.x - positions.element.canvas.x);
			const mismatchY = Math.abs(positions.hoverBorder.canvas.y - positions.element.canvas.y);
			...
			await page.screenshot({
				path: 'test-results/grandchild-hover-border.png',
				fullPage: true
			});
			
			expect(mismatchDistance).toBeLessThan(1); // Hover border should match element position
		} else {
			console.log('Hover border not found!');
			await page.screenshot({
				path: 'test-results/grandchild-hover-border-not-found.png',
				fullPage: true
			});
